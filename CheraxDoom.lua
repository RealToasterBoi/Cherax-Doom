...
-- Companion music watchdog. The MCI sequencer is a process-level device that
-- CheraxDoom cannot stop from its own unload path (every MciSendString on the
-- unload thread returns false), and periodic in-track keep-alive MCI commands
-- are NOT an option (each play command costs a lagspike and degrades the
-- sequencer, per in-game testing). So the stop is delegated to a tiny SEPARATE
-- script in its own Lua state: it survives this script being uninjected, and a
-- live script can reach MCI just fine. It polls the shared feature registry for
-- CheraxDoom's hidden liveness feature; when that leaves, it stops + closes the
-- sequencer for ~2s of frames and then unloads itself.
W.MUSWATCH_SRC = [==[
-- CheraxDoom music watchdog (auto-generated by CheraxDoom.lua; safe to delete).
-- Stops the DOOM MIDI music if CheraxDoom is unloaded, then unloads itself.
local HB = Utils.Joaat("CheraxDoom_Shutdown")   -- CheraxDoom liveness heartbeat
local MY = Utils.Joaat("CheraxDoom_MusWatch")   -- this watchdog's own marker
local dup = nil
pcall(function() dup = FeatureMgr.GetFeature(MY) end)
if dup then
    -- A watchdog is already running; this copy is redundant.
    if SetShouldUnload then pcall(SetShouldUnload) end
else
    local mf = FeatureMgr.AddFeature(MY, "CheraxDoom MusWatch",
        (eFeatureType and eFeatureType.Button) or 0,
        "Internal: stops DOOM music if CheraxDoom unloads, then unloads itself.",
        function() end)
    if mf then
        pcall(function() mf:SetVisible(false) end)
        pcall(function() mf:SetSaveable(false) end)
    end
    local gone = 0
    pcall(EventMgr.RegisterHandler, (eLuaEvent and eLuaEvent.ON_PRESENT) or 7, function()
        local f = nil
        pcall(function() f = FeatureMgr.GetFeature(HB) end)
        if f then gone = 0; return end      -- CheraxDoom alive (or back): stand down
        gone = gone + 1
        if gone > 120 then return end
        if gone == 1 and Logger and Logger.LogInfo then
            pcall(Logger.LogInfo, "[DOOMWAD] muswatch: CheraxDoom gone, stopping music.")
        end
        pcall(Utils.MciSendString, "stop doommus")
        pcall(Utils.MciSendString, "close doommus")
        if gone == 120 then
            pcall(FeatureMgr.RemoveFeature, MY)
            if SetShouldUnload then pcall(SetShouldUnload) end
        end
    end)
...
-- Write + launch the watchdog if one is not already running. Called on every
-- track start (cheap registry lookup), so a manually unloaded watchdog is
-- respawned the next time music begins. Runs on the present thread (playMusic
-- is serviced there), matching how the host launches scripts.
function W.spawnMusWatch()
    if not (Utils and Utils.ExecuteScript and Utils.Joaat
            and FeatureMgr and FeatureMgr.GetFeature) then return end
    local f = nil
    pcall(function() f = FeatureMgr.GetFeature(Utils.Joaat("CheraxDoom_MusWatch")) end)
    if f then return end
    if not W.ensureCacheDir() then return end
    local path = W.cacheDir .. "/muswatch.lua"
    if not W.musWatchWrote then
        if not W.writeBytes(path, W.MUSWATCH_SRC) then return end
        W.musWatchWrote = true
    end
    pcall(Utils.ExecuteScript, (path:gsub("/", "\\")))
end
...
function W.playMusic(mapName)
    ...
    -- The watchdog is the unload protection; the play itself is additionally
    -- bounded to the track length (never unbounded) as a last-resort backstop
    -- for the case where the watchdog was killed too (e.g. unload-all). Costs
    -- ZERO extra MCI commands: the bound rides on the same play command the
    -- track start and each loop already issue. NO mid-track MCI keep-alives.
    W.spawnMusWatch()
    W.musBoundMs = nil
...
-- bladscript resolves SHUTDOWN_HASH from the shared registry and OnClick()s it;
-- that callback runs in THIS script's state (via W.hostShutdown), so
-- SetShouldUnload marks this script for unload, never the host. Standalone it
-- doubles as the liveness heartbeat the music watchdog polls: while this feature
-- is in the registry, CheraxDoom is loaded. Never RenderFeature'd, invisible.
...
if EventMgr and EventMgr.RegisterHandler then
    ...
    -- NOT what silences a standalone uninject: the companion watchdog script
    -- (W.MUSWATCH_SRC) does that from its own live state the moment our liveness
    -- feature leaves the registry, and the track-length play bound is the final
    -- backstop if the watchdog died too.
    pcall(EventMgr.RegisterHandler, (eLuaEvent and eLuaEvent.ON_UNLOAD) or 11, function()
